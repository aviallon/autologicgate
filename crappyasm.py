...
def assembler(data, address_bits=8):
    ...
    for i,l in enumerate(lines):
        ...
        if temp := re.findall("byte\s+([a-zA-Z][a-zA-Z0-9_]+)\s+([\#@]0x[0-9A-Fa-f]{2})", ligne):
            #print(temp)
            variables[temp[0][0]] = temp[0][1]
            ligne = re.split("byte\s+[a-zA-Z][a-zA-Z0-9_]+\s+[\#@]0x[0-9A-Fa-f]{2}", ligne, 1)[1]
        jump = ""
        ...
        if len(ligne) > 0:
            ...
            for k,variant in enumerate(instruction["variants"]):
                ...
                for i in range(len(variant)):
                    ...
                    if cur_arg in registers and (variant[i] == "R" or variant[i] == cur_arg or cur_arg[0] == "U" and variant[i] == "U#"):
                ...
                #print(vals)
                if ok:
                    real_instruction = instruction['instructions'][k][0]
                    for reg in register_names:    
                        real_instruction = real_instruction.replace("_R", "_"+reg, 1)
                    
                    instruction_code = hex(list(instructions.keys()).index(real_instruction))[2:].zfill(8//4)
                    machine_code += [instruction_code]
                    for inst_data in vals:
                        if inst_data in jumplist:
                            machine_code += [inst_data]
                        else:
                            machine_code += [hex(inst_data)[2:].zfill(8//4)]
